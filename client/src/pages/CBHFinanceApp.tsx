    ...
    <Link href="/" className="flex items-center gap-3">
      ...
      {!compact && (
        <span className="leading-none">
          ...
          <span className="block text-xs uppercase tracking-[0.32em] text-slate-500">Private Banking Demo</span>
        </span>
      )}
    </Link>
  );
    ...
    <footer className="border-t border-[#0a1f44]/10 bg-[#0a1f44] text-white">
      <div className="container grid gap-10 py-12 md:grid-cols-[1.2fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-4 max-w-md text-sm leading-6 text-white/70">CBHfinance is implemented here as a controlled demo banking application. It presents professional banking workflows, separated user and admin interfaces, and non-destructive payment behavior for review and testing.</p>
        </div>
        <div>
      ...
      <main>
        <section className="relative overflow-hidden border-b border-[#0a1f44]/10">
          ...
          <div className="container relative grid min-h-[680px] items-center gap-12 py-20 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <p className="mt-5 text-sm text-slate-600">Demo user: emily.johnson@cbhfinance.online · OTP: 246810</p>
            </div>
              ...
              <div className="rounded-[1.5rem] bg-[#0a1f44] p-6 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-2 font-serif text-4xl font-semibold">{dashboard.data ? money(dashboard.data.totalNetWorth) : "Loading"}</p>
                  </div>
        ...
        <section id="security" className="bg-white py-20">
          <div className="container grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
            <div><p className="text-sm font-bold uppercase tracking-[0.3em] text-[#c9a84c]">Security controls</p><h2 className="mt-4 font-serif text-4xl font-semibold">Credential, OTP, timeout, lockout, and audit controls are visible throughout the demo.</h2></div>
...
function LoginPage({ role }: { role: "user" | "admin" }) {
  ...
  const [password, setPassword] = useState(role === "admin" ? "AdminDemo!2026" : "DemoUser!2026");
  ...
  async function submitCredentials(event: FormEvent) {
    ...
    if (result.success) {
      setOtpReady(true);
      setMessage("OTP sent by email. For this controlled demo, use 246810.");
    } else {
    ...
    <div className="min-h-screen bg-[#f8f6f1] text-[#0a1f44]">
      ...
      <main className="container grid min-h-[calc(100vh-5rem)] items-center gap-10 py-16 lg:grid-cols-[0.9fr_1.1fr]">
        <section>
          ...
          <div className="mt-8 rounded-2xl border border-[#c9a84c]/40 bg-white p-5 text-sm text-slate-700"><strong>Demo credentials:</strong><br />{role === "admin" ? "admin@cbhfinance.online / AdminDemo!2026" : "emily.johnson@cbhfinance.online / DemoUser!2026"}<br />OTP: 246810</div>
        </section>
...
function Payments() {
  ...
  async function submit(type: any) { await block.mutateAsync({ paymentType: type, amount: 100, memo: "Demo outgoing payment" }); setModal(true); }
...
function Statements({ rows }: { rows: any[] }) {
  return <Panel title="Monthly Statements"><p className="mb-5 text-slate-600">Monthly PDF statement records are available from January 2025 onward for each account. Download links are secure placeholders for the demo environment.</p><div className="grid gap-3 md:grid-cols-3">{rows.slice(0, 36).map(row => <a key={row.id} href={row.fileUrl} className="flex items-center gap-3 rounded-2xl bg-[#f8f6f1] p-4 font-semibold"><FileText className="h-5 w-5 text-[#c9a84c]" />{row.accountType} · {row.period}</a>)}</div></Panel>;
}
...
function LegalPage({ type }: { type: "terms" | "privacy" | "contact" }) {
  ...
  return <div className="min-h-screen bg-[#f8f6f1]"><MarketingNav /><main className="container py-16"><div className="max-w-4xl rounded-[2rem] bg-white p-8 shadow-sm"><h1 className="font-serif text-5xl font-semibold text-[#0a1f44]">{title}</h1>{type === "contact" ? <form className="mt-8 grid gap-4"><input placeholder="Full name" className="rounded-2xl border px-4 py-3" /><input placeholder="Email" className="rounded-2xl border px-4 py-3" /><textarea placeholder="How can CBHfinance support you?" className="min-h-36 rounded-2xl border px-4 py-3" /><button type="button" className="rounded-full bg-[#0a1f44] px-6 py-4 font-semibold text-white">Submit support request</button><p className="text-sm text-slate-500">This support form is a frontend-only placeholder for this build.</p></form> : <div className="prose prose-slate mt-8 max-w-none"><p>These {title.toLowerCase()} describe the controlled CBHfinance demo environment. The application demonstrates banking-style workflows, credential and OTP flows, statement records, and admin controls for evaluation.</p><p>No outgoing customer payment is processed by this application. Transfer, Wire, ACH, Zelle, and Bill Pay actions are blocked and display the required support message without changing account balances.</p><p>Administrative balance adjustments are recorded with transaction entries and append-only audit log records. The demo is designed for review and should be connected to formal compliance, legal, security, and banking infrastructure before any real-world financial use.</p></div>}</div></main><Footer /></div>;
}
